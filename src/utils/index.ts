...
export function isGlobalSettingsSet(settings: GlobalSettingsInterface | unknown): settings is GlobalSettingsInterface {
  return (settings as GlobalSettingsInterface).accessToken !== undefined;
}

export function isDeviceSetting(settings: DeviceSettingsInterface | unknown): settings is DeviceSettingsInterface {
  return (settings as DeviceSettingsInterface).deviceId !== undefined;
}

export function isSceneSetting(settings: SceneSettingsInterface | unknown): settings is SceneSettingsInterface {
...
// This function converts HEX to Hue, Hue Percent, Saturation (SmartThings uses Hue Percent-Saturation)
...
// This function converts HPercentSL to Hex (L always in 50 ST does not prive it, and  50 is pure color)
export const hslToHex = (hP: number, s: number, l = 50) => {
  const h = (hP * 360) / 100; // Convert Hue Percent to Hue Degrees
  l /= 100;
